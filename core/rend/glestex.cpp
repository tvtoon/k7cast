...
void ReadRTTBuffer()
{
	...
	else
	{
		...
		if (w <= 1024 && h <= 1024)
		{
			...
			TextureCacheData *texture_data = TexCacheGL.getTextureCacheData(tsp, tcw);
...
u64 gl_GetTexture(TSP tsp, TCW tcw)
{
	...
	//lookup texture
	TextureCacheData* tf = TexCacheGL.getTextureCacheData(tsp, tcw);
	...
	else
	{
		...
		{
			TexCacheGL.DeleteLater(tf->texID);
			tf->texID = glcache.GenTexture();
