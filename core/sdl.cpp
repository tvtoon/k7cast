...
static void captureMouse(bool capture)
{
	...
	if (!capture)
	{
		...
		SDL_SetWindowTitle(window, "K7cast");
		mouseCaptured = false;
	}
	else if (SDL_SetRelativeMouseMode(SDL_TRUE) == 0)
	{
		SDL_SetWindowTitle(window, "K7cast - mouse capture");
		mouseCaptured = true;
	}
}
...
bool sdl_recreate_window(u32 flags)
{
	...
	window = SDL_CreateWindow("K7cast", x, y, window_width * scaling, window_height * scaling, flags);
	if (window == nullptr)
	{
